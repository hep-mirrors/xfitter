...
C--   - Combine FETCHa and GINTRP to speed up by using less PDF calls.
...
C--   Output variables: f2,f2c,f2b,fl,flc,flb.
      SUBROUTINE MSTWNCa(x,q,ipn,f2,f2c,f2b,fl,flc,flb)
     ...
     &     mCharm,mBottom,alphaSQ0,alphaSMZ,alphaSorder,alphaSnfmax
      ...
      COMMON/GRPTHYa/FLAVOR
      COMMON/DYLAMBa/xlam,S0
      COMMON/iordCommona/iord
      COMMON/GAUS96a/XI(96),WI(96),XX(97),NTERMS ! G.W. 15/02/2007
...
!$OMP THREADPRIVATE(/TRprimeCommon/,/GRPTHY/)
      ...
      IF (IORD.EQ.2) THEN
         CALL MSTWNCnnloa(x,q,ipn,f2,f2c,f2b,fl,flc,flb)
         RETURN
...
C--   End of MSTWNC subroutine.
      ...
      SUBROUTINE MSTWNCnnloa(x,q,ipn,f2,f2c,f2b,fl,flc,flb)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
      DIMENSION FTEMP(5)
      INTEGER alphaSorder,alphaSnfmax
      DOUBLE PRECISION distance,tolerance,
...
c$$$C--   Rewrite FETCHa to return PDFs from grids.
c$$$      SUBROUTINE FETCHa(X,S,IPN,FTEMP)
...
C--   G.W. 11/04/2012 Combine FETCHa and GINTRP to use less PDF calls.
      SUBROUTINE FETCHa(X,S,IPN,FTEMP)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z) ! G.W. 15/02/2007
      DIMENSION FTEMP(5)
      COMMON/GRPTHYa/FLAVOR
      COMMON/DYLAMBa/xlam,S0 ! G.W. 17/07/2007
      CHARACTER prefix*50,prefix1*50,cl*4
      INTEGER ISET
      COMMON/mstwfilesa/iset,prefix,cl
...
!!        CALL HF_GET_PDFS(x,xmuf2,PDFSF)
      call acot_get_pdfs(x, xmuf, PDFSF)   !! use pointer-function instead
...
C--   Swap usea and dsea.
      ...
      ELSE IF (IPN.NE.1) THEN
         WRITE(6,*) "Error in FETCHa, IPN = ",IPN
         STOP
      END IF
...
!!      alpha = hf_get_alphas(qs) 
      alpha = acot_get_alphaS(sqrt(qs)) !! use interface instead
