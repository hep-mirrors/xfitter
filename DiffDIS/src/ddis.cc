...
void dDIS_t::Configure(const string& gridName) {
	...
	PomPdf = dPdf;
	// cout << "Pomeron flux: " << dPdf->Pflux;
	// cout << "Reggeon flux: " << dPdf->Rflux;
  Pflux = dPdf->Pflux;
  Rflux = dPdf->Rflux;
	...
	// if(CCards.GetIntParam("alphas_RG")) alphas = new AlphaS_RG;
	// else alphas = new AlphaS_Lam;
...
dDIS_t::dDIS_t(const string& gridName) {
	...
	//JSsetFact(1, 4);
	JSsetFact(1, 0);
	//JSsetContr(1, 0, 1); //--- born, gluon, quark
	JSsetContr(1, 1, 1); //--- born, gluon, quark, i.e. LO + NLO

	//--- TEST -- passed
	//StrFns::Initialize(pxf, 0);
	
	#ifdef HI_TWISTS
    // xFLt4 = new xFLt4_t(4, Lambda4tr, Twist4Model, CCards.GetRealParam("Tw4_QQ0"));
	#endif
  
	cout << "Pomeron flux: " << Pflux;
	cout << "Reggeon flux: " << Rflux;
}
