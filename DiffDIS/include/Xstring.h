/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  2012-02-15
    Token return first token and chops it off *this.
    Remainder removed.
...
private:
  ...
  bool Match_qm(const string& qs, size_type start) const {
    if(qs.find('?') == string::npos) return !compare(start, qs.size(), qs);
    ...
    string Q(qs);
    int i,j;
    // for(*qp = Q.c_str(); *qp; qp++, sp++) if(*qp != '?' && *qp != *sp) return false;
    for(i=j=0; i < qs.size(); i++, j++) if(qs[i] != '?' && qs[i] != S[i]) return false;
    return true;
  }
...
public:
  ...
  bool Match(const string& glob_patt, bool case_sensitive = true) const {
    // --- glob_patt is a glob style pattern
