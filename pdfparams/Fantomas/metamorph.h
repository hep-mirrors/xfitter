 ...
 *  of the form
 *       xf(x) = Carrier(x, Sc)*Modulator(x, Sm),
 *  where 
 *   Carrier defines the asymptotic behavior of xf(x) in the limits x->0 and x->1;
 *   Modulator(x,Sm) determines the behavior of xf(x) over the interval 0 < x < 1.
 *
 ...
 *     that set the value of xf(x) at Ncp user-selected control points CP at discrete x values 
 ...
 *     changed to update the vector C before the function xf(x) is called. 
 */
{
...
public:
  ...
  // for varied x values. We write the PDF function as xf(x_i) = Fc(x_i) * Fm(x_i),
  ...
  double Chi2prior();
  //Returns the contribution to the objective function associated with the prior on this
  //metamorph
