...
public:
  ...
  double Chi2prior();
  //returns a contribution to the objective function associated with the priors
  //on the metamorphs

  inline void SetVerbosity(const unsigned int VerbosityLevelIn=0){
  //Sets verbosity level for printing out diagnostics  
    VerbosityLevel=VerbosityLevelIn;    
  }

  void MetamorphDiagnostics(const std::string& outputDirectory="./");
  //For each metamorph, prints/writes momentum fractions, modulator parameters, Bernstein coefficients

  void Chi2Diagnostics(const std::string& outputDirectory="./");
  //Prints/writes diagnostics from prior penalties on metamorphs
