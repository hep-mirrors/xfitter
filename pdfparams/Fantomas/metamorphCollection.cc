...
void metamorphCollection::PushMember()
{
  ...
  for (int i = 0; i < k; i++) {
	...
	cout << "Using metamorph::xf(x) = Carrier(x) for control point." << endl;
      }
...
void metamorphCollection::UpdateParameters(const int ifl, double *deltas)
{
  ...
  int paraiMet = PositionRoster[ifl];
...
void metamorphCollection::MetamorphDiagnostics(const string& outputDirectory)
{
  ofstream fantoCout;
  double totalmom=0.0;

  string fname=outputDirectory+"fantomas_functional_parameters.txt";
  ...
  fantoCout << "# Parameters of the functional from each metamorph" << endl;
  fantoCout << "# xf(x) = <xf>*x^Sc1*(1-x)^Sc2*(1+fbezier(x))" << endl <<endl;
  
  for (int i = 0; i < NMeta; i++){
    // Write momentum fractions and parameters of each metamorph
    fantoCout << flvcomment[i][1] << endl;
    double xmom=MellinMoment(iflavor[i],0);
    totalmom +=xmom;
    fantoCout << "<xf> = " << xmom << endl;
    fantoCout << "Carrier parameters:"<<endl;
    
    for (int j = 1; j < maxSc; j++)
      fantoCout << "Sc[" << j << "] = " << Scm[i][j] << endl;
    fantoCout << "Values of control points:"<<endl;
    
    for (int j = maxSc; j < maxSc + Nm[i]+1; j++)
      fantoCout << "Sm[" << j << "] = " << Scm[i][j] << endl;

    fantoCout << "Bernstein coefficients:"<<endl;    
    for (int j = 0; j < Nm[i]+1; j++) 
      fantoCout << "C[" << j << "] = " << MetaVector[i].Cs(j) << endl;                  
    fantoCout << "Condition number ="<< MetaVector[i].GetConditionNumber() << endl;
    fantoCout << endl;
  }//for (int i = 0

  fantoCout << "Total momentum fraction = "<<totalmom <<endl;
  
  fantoCout.close();
} // metamorphCollection::MetamorphDiagnostics ------------------------------------
