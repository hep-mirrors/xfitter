...
extern "C" {
  int z_set_etabc__(void *mass_range, void *y_range, double *pel_pt_cut, 
      int *pneb, void *eb );
  int z_get_etabc_xs__(void *xs);
  int z_free_etabc__();
}
...
int iy(int imp, double *pars, double *iyval){
  ...
  //  1           |   0 |   1
  //  -1          |   2 |   3
  get_pdfconv(m, y_low, 1., scale, xfxc[0], xfxc[1]);
  get_pdfconv(m, y_low, -1., scale, xfxc[2], xfxc[3]);
  for (int ib=0;ib<neb;ib++){
  ...
  for (int iyb=0;iyb<nyb;iyb++){
    ...
    get_pdfconv(m, yb, 1., scale, pcb[0], pcb[1]);
    get_pdfconv(m, yb, -1.,scale, pcb[2], pcb[3]);
    get_pdfconv(m, ym, 1., scale, pcm[0], pcm[1]);
    get_pdfconv(m, ym, -1.,scale, pcm[2], pcm[3]);
...
double ai_c(const double &m, const double &ry, const int cdf,
  ...
  double y =dir*ry;
  double c_eta_lab = dir*(rc_low_eta_lab + rc_up_eta_lab)/2.;
  double g = cosh(y); // gamma, lorentz factor
  double b = tanh(y); // beta, velocity

  // for eta defined cuts
  double c_eta_cms = Utils::costh_LT ( b, g, c_eta_lab);
  ...
  if ( c_eta_cms < c_low_pt_cms || c_eta_cms > c_up_pt_cms ) return 0.;

  double a1(0.), a2(0.), ai(0.);
  a1 = 1.+pow(c_eta_cms,2); //  c_up_cms + pow(c_up_cms,3)/3. - ( c_low_cms + pow(c_low_cms,3)/3.);
  a2 = c_eta_cms; //(pow(c_up_cms,2)-pow(c_low_cms,2))/2.;
...
int z_set_etac__(void *mass_range, void *y_range, double *pel_pt_cut, 
    int *pneb, void *eb ){
  using namespace IntZ_EBC;
  neb = *pneb;
...
//start = clock();
  ...
  double ypt_pars[2];
  ypt_pars[0] = IntZ_EBC::yr[0]; ypt_pars[1] = IntZ_EBC::yr[1];

  for (int ib=0; ib<IntZ_EBC::neb; ib++){
    IntZ_EBC::cb_sig_em[ib] = 0.;
  }
  
  int integ_res = 
    IntZ_EBC::integrate_M(ypt_pars, IntZ_EBC::mr[0], IntZ_EBC::mr[1]);

//finish = clock();
//cout << double(finish - start)/CLOCKS_PER_SEC<< endl;
  for (int ib=0;ib<IntZ_EBC::neb;ib++){
    *((double*)xs+ib) = IntZ_EBC::cb_sig_em[ib];
  }
