...
extern "C" {
  int wpenu_integ__(double *integral, double *integral_err, void *mass_range, 
...
namespace IntegDY_Wp
{

double intgd_m(double , void *);
//double intgd_y(double , void *);
double dsig_Wp(double , void *);
double get_PhiWp(const double &, const double &, const double &);
...
double get_PhiWp(const double &x1, const double &x2, const double &scale)
{
  double xq1[13];
  double xq2[13];
  int   ichk = 0;
...
double dsig_Wp(double y, void *mcpt_pars){
...
int wpenu_integ__( double *integral, double *integral_err, void *mass_range, 
  ...
  gsl_function gF;
  gF.function = &(IntegDY_Wp::intgd_m);
  gF.params = &ycpt_pars;
  
  int integ_res = 
    gsl_integration_qags(&gF, m_low, m_up, 0, IntegDY_Wp::INT_PREC, 
...
void dywp_check__() {
  ...
  for ( int i = 0; i<=0; i++){
    mass += i / 10.;
    double val = IntegDY_Wp::dsig_Wp(0.0 , &mass);
  ...
  int res = wpenu_integ__(&integral,&err,mass_range,y_range,eta_range,&pt_cut);
  /*
  double yrange[2]; 
  yrange[0] = -10.;
  yrange[1] =  10.;
