...
extern "C" {
  int z_set_ybins__(void *mass_range, void *y_range, double *pel_pt_cut, 
      int *pneb, void *eb);
...
//start = clock();
  ...
  double ypt_pars[2];
  ypt_pars[0] = IntZ_YB::yr[0]; ypt_pars[1] = IntZ_YB::yr[1];

  for (int ib=0; ib<IntZ_YB::neb; ib++){
    IntZ_YB::cb_sig_em[ib] = 0.;
  }
  
  int integ_res = 
    IntZ_YB::integrate_M(ypt_pars, IntZ_YB::mr[0], IntZ_YB::mr[1]);

//finish = clock();
//cout << double(finish - start)/CLOCKS_PER_SEC<< endl;

  for (int ib=0;ib<IntZ_YB::neb;ib++){
    *((double*)xs+ib) = IntZ_YB::cb_sig_em[ib];
  }
