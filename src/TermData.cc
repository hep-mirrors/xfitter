...
void splitTermInfo(map<string,string>&out,const char*s){
  ...
  while(true){
    ...
    while(*p==' ')++p;//skip whitespace leading to key
    if(*p==0)return;
    if(*p==':'||*p==';'){
      cerr<<"[ERROR] No key before separator in TermInfo=\""<<s<<"\" (position "<<p-s<<")\n"
            "Expected format: TermInfo='key1=value1;key2=value2;...'"<<endl;
    ...
    k1=p-1;
    ++p;
    while(*k1==' ')--k1;//trim trailing whitespace in key
    ++k1;
    while(*p==' ')++p;//skip whitespace leading to value
    if(*p==0||*p==':'||*p==';'||*p=='='){
    ...
    v1=p-1;
    if(*p)++p;
    while(*v1==' ')--v1;//trim trailing whitespace in value
    ++v1;
...
YAML::Node getFromByReaction(const string&parName,const ReactionTheory*const reaction){
  ...
  if(byReactionNode.IsMap()){
  ...
  }else{//Check in case somebody still uses the old syntax
    ...
    if(once){
      ...
      if(oldstyleNode.IsDefined()){
        ...
        "It seems you are using the old syntax to pass reaction-specific parameters. By the new syntax, which was introduced during the \"TermData rewrite\", reaction node should be under \""<<BY_REACTION<<"\", like this:\n"
        ...
        "This type of warning will not be issued again."<<endl;
...
enum class Type{None=0,DoublePtr=1,String=2,Int=3,Array=4};//None is unused
...
enum class ParamScope{Term=0,Reaction=1,Global=2};
...
string TermData::getParamS(const string&parName){
  ...
  {
  string out;
  if(getStringFromTermOrReaction(parName,&out)==0)return out;
  }
  //3. Global from YAML
...
const double*TermData::getParamD(const string&parName){
  ...
  if(it!=term_info.end()){
    ...
    if(itp!=gParameters.end())return itp->second;
...
bool TermData::hasParam(const string&parName){
   //1. TermInfo from datafile
  if(term_info.count(parName)>0)return true;
  {//2. Reaction-specific from yaml steering
  YAML::Node parNode=getFromByReaction(parName,reaction);
  if(parNode.IsDefined())return true;
  }
  {//3. Global from YAML
  YAML::Node parNode=rootNode[parName];
  if(parNode.IsDefined())return true;
