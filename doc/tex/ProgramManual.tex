...
%%%%
The description of two examples fitting different data is provided in this section:
In first example the fit is performed with HERA DIS inclusive data alone and in second 
all available data in \fitter\ is fitted simultaniously. 

\subsection{DIS inclusive only}
The default {\tt steering.txt} file contains HERA inclusive DIS data.
The result of fitting with default parameters in {\tt steering.txt} and 
{\tt minuit.in.txt} steering files, the fit result which user obtains
can be compared to saved output in "output\_examples".

%%%%
\subsection{All processes}
In order to fit all available data sets in \fitter\ user first has to copy 
corresonding {\tt steering.txt} from "input\_steering": 
\goodbreak                 
