...
\item  {\tt QCDNUM}: (Optional) Namelist to modify \qcdnum evolution parameters such as $x$ and $Q^2$ grids.
...
\item {\tt RunningMode = 'LHAPDF Analysis'} -- uses PDFs from the \lhapdf library to produce predictions for the
...
The $\chi^2$ function definitions and {\fitter} implementation are given in \ref{sec:chi2}.
Briefly, different  options for various $\chi^2$ terms are controlled by the flag {\tt 'CHI2SettingsName'}
to specify the term and by the flag {\tt Chi2Settings} to specify the options. 
 {\tt 'CHI2SettingsName'} can have following values: 
\begin{itemize}
\item {\tt 'StatScale', 'UncorSysScale', 'CorSysScale'} for statistical, uncorrelated, and correlated
systematic uncertainties. Their scaling properties can be {\tt 'NoRescale'}, {\tt 'Poisson'} or {\tt 'Linear'}
\item {\tt 'UncorChi2Type', 'CorChi2Type'} control treatment of systematics uncertainties correlation. They 
can take values of 'Diagonal', 'Hessian'   
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \bf {\minuit steering cards}\rm
    ...
    \item{\tt  Reaction}    --- (string) reaction type of the data set. Reaction type is used 
                      ...
                      \begin{itemize}
                        ...
                        \item {\tt 'NC pp'}    -- single differential $d \sigma_Z / d y_Z$ at $pp$ and
                                      $p\bar{p}$ colliders
                                      ({\tt LO} with k-factors and \applgrid interface)
                        ...
                        \item 'FastNLO jets' -- jet cross sections using \fastnlo interface.
                                     All $ep$, $pp$ and $p\bar{p}$ colliders are supported.

                        \item 'FastNLO ep jets normalised' -- jet cross sections in the $ep$ collisions 
                                     using \fastnlo interface and normalised to the inclusive DIS cross sections.
...
\item[virtgrid] can be used if the fit is performed on the multidimensional 
  ...
  represented as a linear array -- a sequence of \applgrid grids for \(Z\), each
...
\end{verbatim}
  ...
  The minimization information from the \minuit program is stored using the standard \minuit output format in the {\tt output/minuit.out.txt}
  file. The level of verbosity for this information can be changed by \minuit commands
  ...
  In addition the \fitter\ package provides PDFs in the \lhapdf format as {\tt output/lhapdf.block.txt} file. 
  To obtain the
  \lhapdf grid file, run the {\tt tools/tolhapdf.cmd} script. The script provides a
