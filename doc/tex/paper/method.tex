...
In this section we describe the available options in \fitter.
...
functional form and different flavour decomposition.
...
The form of polynomials $P_i(x)$ depend on the style, defined as a steering parameter. 
...
based on generating replica set by introducing Gaussian fluctuations on the central PDF set with a variance determined by the PDF uncertainty given by the eigenvectors. Both reweighting methods are implemented in \fitter.



The Bayesian Reweighting technique relies on the fact that the MC replicas of a PDF sets (i.e. NNPDF) give a representation of the probability distribution in the space of PDFs. In
particular, the PDFs are represented as ensembles of $N_{\rm rep}$ equiprobable ({\em i.e.} having all weight
equal to unity) replicas. The central value for a given observable, $\mathcal{O}(\mathrm{PDF})$, is 
computed as the average of the predictions obtained from the ensemble as
\begin{equation}
...
\begin{equation}
\langle\mathcal{O}(\mathrm{PDF})\rangle =  \frac{1}{N_{\mathrm{rep}}} \sum_{k=1}^{N_{\mathrm{rep}}} w_k \mathcal{O}(\mathrm{PDF}_k).
\end{equation}
 To simplify the use of reweighted set, an unweighted set ({\em i.e.} a set of equiprobable 
replicas which incorporates the information of the original weights) is generated using the method described
in~\cite{Ball:2011gg}. 
