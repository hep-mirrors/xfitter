...
Performing a QCD analysis it is necessary to check stability of the results
...
In this section we describe the available option for the fit methodology in \fitter.
...
The PDFs are parametrised using several predefined
...
In \fitter, following functional forms to parametrise PDFs can be used:
%using free parameters to be extracted from the fit:
%\begin{description}
\paragraph{Standard Polynomials:} \rm
The standard polynomial form is most commonly used by the PDF groups.
A polynomial functional form is used to parametrise the $x$-dependence of the PDFs, where index $j$ denotes each parametrised PDF:
%The term standard is understood to refer to a simple polynomial 
%that interpolates between the low and high $x$ regions:
\begin{equation}
 xf_j(x) = A_j x^{B_j} (1-x)^{C_j} P_i(x).
\label{eqn:pdf_std}
\end{equation}
...
The form of polynomials $P_i(x)$ depend on the style and is defined as a steerable parameter. 
The form $(1 + \epsilon_j \sqrt{x} + D_j x + E_j x^2)$
is used for the HERAPDF \cite{h1zeus:2009wt} style
with additional constraints relating to the flavour decomposition of the 
light sea. 
For the CTEQ style, $P_i(x)$ takes the form $e^{a_3x} (1 + e^{a_4} x + e^{a_5} x^2)$ and,
in contrast to polynomial form, is positive by construction.
...
\paragraph{Bi-Log-Normal Distributions:} \rm
...
\paragraph{Chebyshev Polynomials:} \rm
A flexible parametrisation employed for the gluon and sea distributions
...
For better modelling the low-$x$ asymptotic of those PDFs, the polynomial of the argument $\log(x)$ are considered.
...
parametrisatons were studied in Ref.~\cite{Chebyshev}. Fig. \ref{fig:cheb} (taken from \cite{Chebyshev})
shows the comparison of the gluon density obtained with the parametrisation Eqs.~\ref{eq:glu}, \ref{eq:sea} to 
...
Fig. \ref{fig:pdfs} illustrates the comparison of the PDFs accessed from \lhapdf as produced with the drawing 
...
\end{eqnarray}
where the experimental uncertainties are given in a form of covariance matrix $C_{i,k}$ for measurements in bins $i$ and $k$.
...
\end{description}
Any source of the measurement systematic uncertainty can be treated as additive or multiplicative. 
...
%
the $\chi^2$ function for the central fit for which only
...
fit are statistically independent, they are combined in quadrature to arrive at the total 
...
is often more practical to represent PDF uncertainties.
As it was illustrated by \cite{Gao:2013bia}, it is possible to transform MC to eigenvector
representation. Tools to perform this transformation are provided with \fitter and were 
recently employed to obtain correlated sets of PDFs at different perturbative order \cite{hfcorrpaper}.
%
\end{description}
%
...
As an alternative to performing a full QCD fit, \fitter allows to assess the impact of including new
...
Bayesian Reweighting was first proposed for the PDF sets delivered in form of MC replicas ensembles 
by~\cite{Giele:1998gw} and further developed by the NNPDF Collaboration~\cite{Ball:2011gg,Ball:2010gb}. 
...
The Bayesian Reweighting technique relies on the fact that the MC replicas of a PDF sets (i.e. NNPDF) give 
...
to Bayes Theorem and the weight of each replica, $w_k$, is updated according to
\begin{equation}
...
%\end{equation}
%}
Given a PDF set and a corresponding set of weights, which describes the impact on the same set of the
inclusion of new data, the prediction for a given observable can be computed as the {\em weighted} average,
\begin{equation}
...
the information of the original weights) is generated according to the unweighting procedure described in~\cite{Ball:2011gg}. 
In this respect, it is useful to recall that the number of effective replicas of a reweighted set is measured by its Shannon 
...
\end{equation}
which corresponds to the size of a refitted equiprobable replicas set containing the same amount of information. 
As such, the number of effective replicas, $N_\mathrm{eff}$, gives an indicative measure of the optimal size of an 
