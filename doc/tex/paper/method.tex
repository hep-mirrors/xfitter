...
light sea. This parametrization is termed HERAPDF-style. The polynomial can also
be parametrized in the CTEQ-style, $P_i(x)$ takes the form $e^{a_3x} (1 + e^{a_4} x + e^{a_5} x^2)$ and,
...
Polynomials with argument $\log(x)$ are considered .or better modelling the low-$x$ asymptotic of those PDFs. The polynomials are multiplied
...
are provided in the eigenvectors representation has been also developed~\cite{Watt:2012tq}. The latter is 
...
$N_\mathrm{eff}$.  Clearly if $N_\mathrm{eff}$ is much smaller than the original number of replicas the new data have great impact, but it is unreliable to use the new reweghted set. Instead a full refit should be performed.
