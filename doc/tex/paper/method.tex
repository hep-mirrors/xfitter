...
The PDFs are parametrised at the chosen starting scale required to be below charm mass threshold 
by the set of default defined PDFs in \fitter.
In \fitter various functional forms to parametrise PDFs can be tested:
...
\label{eqn:pdf_std}
\end{equation}
The standard polynomial form is most commonly used by PDF groups.
...
The nuisance parameters $b_j$ as well as the PDF 
...
%
Instead, the correlated systematic uncertainties of the data are then used to estimate 
the errors on the PDF parameters as follows:
The cross section is varied by $\pm 1 \sigma$ shift from the central value 
...
those from the Hessian method, as the offset method does not use the information on correlated systematic uncertainties 
in the central fit.
...
extract the PDF set. The PDF central values and experimental
