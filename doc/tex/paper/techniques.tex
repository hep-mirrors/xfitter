...
\item{\bf{}Fast grid technique:}
  ...
  only once for the set of interpolating functions the time consuming
  higher-order calculation involving complicated convolutions, see
  Eq.~\ref{eq:fact}. The repetition of a cross section evaluation for
...
\begin{itemize}
  \item The \fastnlo project~\cite{Kluge:2006xs} has been interfaced
    ...
    Since it is completely sufficient in most cases, the original
    \fastnlo tables of version~1.4 could be evaluated with arbitrary
    scale factors for the renormalisation scale \mur, but only with a
    fixed set of predefined scale factors for \muf. The significantly
    updated version 2 of \fastnlo~\cite{Britzger:2012bs} allows in
    addition to the less space consuming tables with fixed \muf
    factors the creation of tables, where \mur and \muf can be chosen
    ...
    downloaded as well.  The latest release is compatible with \lhapdf
...
\item The \applgrid package~\cite{Carli:2010rw}, which is also
    available from~\cite{APPLGRID:HepForge}, implements in addition to
    the jet cross section from \nlojetpp calculations of DY
    production. The look-up tables (grids) can be generated with
    modified versions of the MCFM parton level generator for
    DY~\cite{Campbell:1999ah,Campbell:2000je,Campbell:2010ff}.  For
    NNLO predictions $k$-factors can be applied.
    ...
    illustration of ATLAS PDFs extracted using $k$-factors is shown in
    Fig.~\ref{fig:atlas} togehter with the comparison to global PDF
