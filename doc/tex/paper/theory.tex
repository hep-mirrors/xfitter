...
The formalism relating DIS measurements to pQCD and the PDFs has been described
 in detail in many extensive reviews ~\cite{chooseone} {\bf put a reference her do not try to do the whole job} and will only be briefly recapped here.
...
transferred to the hadronic vertex. {\bf but you need to define x and y in terms of kinematic variables more clearly- use any HERA paper}
\\
%
...
in terms of structure functions and in LO the $e^+p$ and $e^-p$ cross sections are sensitive to different quark flavour 
densities:
\begin{eqnarray}
%  \nonumber
    \begin{array}{rll}
...
\end{eqnarray}
%
Beyond leading order the QCD predictions for the DIS structure functions are obtained by convoluting 
the PDFs with the coefficient functions (hard process matrix elements) 
calculated using various schemes which differ in their treatment of 
heavy quark production, i.e.
...
\\
The following VFN schemes are considered in \fitter\ :
...
In addition, the zero-mass variable flavour number scheme (ZM-VFNS) in which 
heavy quark densities are included in the proton for $Q^2>>m_h^2$ but are treated 
as massless in both the initial and final states is also available in \fitter\ .
...
%
The Thorne-Roberts (TR) scheme smoothly connects low
scales below ($Q^2<m_h^2$), where a fixed order calculation of heavy quark
 production from boson-gluon fusion is made accounting for the heay quark mass,
...
and TR optimal~\cite{Thorne:6180}, with a smoother transition across the heavy quark threshold 
...
QCDNUM implementation or through the interface to the open-source code OPENQCDRAD (ABM)~\cite{openqcdrad:page}.
...
and the NNLO QCD corrections to the massive Wilson coefficients 
are available for NC to the currently best known approximation~\cite{SMoch:npb864}.
\end{description}
...
predictions for DY production in \fitter\: at LO an analytic calculation 
...
PDF{\bf find a reference for this} and can thus increase the precision of the 
...
calculations are now quite advanced {\bf find a reference for this,e.g.Nigel 
Glover}. Within \fitter\ the programmes MCFM and 
