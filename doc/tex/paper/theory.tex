...
the PDFs with the respective coefficient functions (hard process matrix elements). The DIS measurements span from low to high $Q^2$, such that  
...
In \fitter this scheme is available for the DIS structure function calculation via interface to the \qcdnum package.
...
Within \fitter programs such \texttt{MCFM} and 
