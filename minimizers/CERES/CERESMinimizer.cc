...
void CERESMinimizer::doMinimization() {
  ...
  if (ceresNode["threads"]) {
    auto diffCostFunction = new CostFuntionrDataAndDerivative<CostFunctorData>();
  ...
  for (int i = 0; i < npars; i++) {
    char val[15];
    char err[15];
    snprintf(val, sizeof(val), "%.6f", parVals[i]);
    snprintf(err, sizeof(err), "%.6f", sqrt(covmat[i*npars+i]));
