...
c Longitudinal: equation (19) in PLB347 (1995) 143 - 151
      ...
      implicit none
      real*8 xi, pi, term1, fii_hq, fjj_hq
      parameter (pi = 3.14159265359d0)
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      asymp_l = 1.d0/6.d0/pi*(4.d0/xi - 4.d0/3.d0*term1 
     #     + (1.d0 - 2.d0/xi - 1.d0/6.d0*term1)*fjj_hq(xi)
     #     - (3.d0/xi + 0.25d0*term1)*fii_hq(xi))
      return
      end
...
c transverse mass factorization: equation (22) in PLB347 (1995) 143 - 151
     ...
     #     - (1.d0 + 2.d0/xi + 0.25d0*term1)*fjj_hq(xi))
      return
      end
c equation (24) in PLB347 (1995) 143 - 151
      real*8 function fii_hq(xi)
      implicit none
      real*8 pi, term1, term2, xi, dilog2
      parameter (pi = 3.14159265359d0)
      term1 = dsqrt(xi)
      term2 = dsqrt(4.d0 + xi)
      fii_hq = 4.d0/term1/term2*(-pi*pi/6.d0 
     #      - 0.5d0*(dlog((term2 + term1)/(term2 - term1)))**2
     #      + (dlog(0.5d0*(1.d0 - term1/term2)))**2 
     #      + 2.d0*dilog2(0.5d0*(1.d0 - term1/term2)))
      return
      end
c equation (23) in PLB347 (1995) 143 - 151
      real*8 function fjj_hq(xi)
      implicit none
      real*8 pi, xi, term1, term2
      parameter (pi = 3.14159265359d0)
      term1 = dsqrt(xi)
      term2 = dsqrt(4.d0 + xi)
      fjj_hq = 4.d0/term1/term2*dlog((term2 + term1)/(term2 - term1))
      return
      end
...
c---------------------
      real*8 function c2nlobarq(eta,xi)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
c---------------------
      real*8 function c2nlobarg(eta,xi)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
c---------------------
      ...
      cla=cg*tr*(cla + beta*asymp_l(xi) + rho*thresha_l(eta,xi))
...
c---------------------
      ...
      cta=cg*tr*(cta + beta*asymp_t(xi) + rho*thresha_t(eta,xi))
...
c---------------------
      ...
      clnlobarg=cg*tr*(cl+beta*asympbar_l(xi)+rho*threshbar_l(eta,xi))
...
c---------------------
      ...
      ctnlobarg=cg*tr*(ct+beta*asympbar_t(xi)+rho*threshbar_t(eta,xi))
...
c For QCD take tf = 1d0/2d0, 
c for QED take tf = 1d0 
c eta = (s - 4d0*m2)/4d0/m2, s is the gamma* gluon (gamma) CM Energy
c xi = Q^2/m2
      ...
      beta = dsqrt(eta/(1.d0+eta));
      Lbeta = dlog((1.d0+beta)/(1.d0-beta))
      dborn_t = 2.d0 * tf * pi * (1.d0-beta**2) * xi *
     ~  (-32.d0*beta**3*Lbeta + 32.d0*beta**5*Lbeta 
     ~     + xi**2*beta**6 - xi**2*beta**4 + 8.d0*xi*beta**4 
     ~     - 64.d0*beta**4 - xi**2*beta**2 - 16.d0*xi*beta**2 
     ~     + 80.d0*beta**2 + xi**2 + 8.d0*xi + 16.d0)*
     ~     (-4-xi+xi*beta**2)**(-3)/beta
      return
      end
c------------------------------------
...
c see above for additional comments
      ...
      beta = dsqrt(eta/(1.d0+eta));
      Lbeta = dlog((1.d0+beta)/(1.d0-beta))
      dborn_l = 32.d0 * tf * pi * (1.d0-beta**2)**3 * xi**2 *
     ~  Lbeta * (-4.d0-xi+xi*beta**2)**(-3)
