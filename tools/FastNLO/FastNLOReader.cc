...
void FastNLOReader::CalcReferenceCrossSection( ){
  ...
  XSectionRefMixed.resize(13);		  
  XSectionRefQ2.resize(13);
  XSectionRefMufQ2MuRMixed.resize(13);

  if ( BlockB_LO_Ref && BlockB_NLO_Ref ){

    for(int i=0;i<NObsBin;i++){
      for(int l=0;l<BlockB_LO_Ref->NSubproc;l++){ 
	XSectionRef[i] +=  BlockB_LO_Ref->SigmaTilde[i][fScalevar][0][0][l];
      }
      for(int l=0;l<BlockB_NLO_Ref->NSubproc;l++){ 
	XSectionRef[i] +=  BlockB_NLO_Ref->SigmaTilde[i][fScalevar][0][0][l];
      }
    }
  ...
  if ( BlockB_LO->NScaleDep == 3 ){
	...
	XSectionRefMixed[i]		+= BlockB_LO ->SigmaRefMixed[i][n];
	XSectionRefQ2[i]		+= BlockB_LO ->SigmaRefQ2[i][n];
	XSectionRefMufQ2MuRMixed[i]	+= BlockB_LO ->SigmaRefMufQ2MuRMixed[i][n];
      }
      for(int n=0;n<BlockB_NLO->NSubproc;n++) {
	XSectionRefMixed[i]		+= BlockB_NLO->SigmaRefMixed[i][n];
	XSectionRefQ2[i]		+= BlockB_NLO->SigmaRefQ2[i][n];
	XSectionRefMufQ2MuRMixed[i]	+= BlockB_NLO->SigmaRefMufQ2MuRMixed[i][n];
      }
    }
  }
...
void FastNLOReader::CalcCrossSection( ){
  ...
  for(int i=0;i<NObsBin;i++){
	...
	for(int l=0;l<BlockB_LO->NSubproc;l++){ 
	  XSection[i]		+=  BlockB_LO->SigmaTilde[i][fScalevar][j][k][l] *  BlockB_LO->AlphasTwoPi_v20[i][scalenode2]  *  BlockB_LO->PdfLc[i][scalenode2][k][l];
	  XSection_LO[i]	+=  BlockB_LO->SigmaTilde[i][fScalevar][j][k][l] *  BlockB_LO->AlphasTwoPi_v20[i][scalenode2]  *  BlockB_LO->PdfLc[i][scalenode2][k][l];
	...
	for(int l=0;l<BlockB_NLO->NSubproc;l++){ 
	  XSection[i]		+=  BlockB_NLO->SigmaTilde[i][fScalevar][j][k][l] *  BlockB_NLO->AlphasTwoPi_v20[i][scalenode2]  *  BlockB_NLO->PdfLc[i][scalenode2][k][l];
	}
      }
    }
	...
	for(int iNodeF=0;iNodeF<BlockB_LO->NscalenodeScale2;iNodeF++){
	  for(int x=0;x<nxmax;x++){ 
	    ...
	    for(int n=0;n<BlockB_LO->NSubproc;n++){ 
	      XSection2Scales[i]	+=  BlockB_LO->SigmaTilde2Scales[i][fScalevar][fScalevar][jNodeR][iNodeF][x][n] * BlockB_LO->AlphasTwoPi2Scales[i][jNodeR]   *  BlockB_LO->PdfLc2Scales[i][iNodeF][x][n];
	      XSection2Scales_LO[i]	+=  BlockB_LO->SigmaTilde2Scales[i][fScalevar][fScalevar][jNodeR][iNodeF][x][n] * BlockB_LO->AlphasTwoPi2Scales[i][jNodeR]   *  BlockB_LO->PdfLc2Scales[i][iNodeF][x][n];
	    }
	    // NLO Block
	    for(int n=0;n<BlockB_NLO->NSubproc;n++){ 
	      XSection2Scales[i]	+=  BlockB_NLO->SigmaTilde2Scales[i][fScalevar][fScalevar][jNodeR][iNodeF][x][n] * BlockB_NLO->AlphasTwoPi2Scales[i][jNodeR]   *  BlockB_NLO->PdfLc2Scales[i][iNodeF][x][n];
	    }
	  }
	}
      }
	...
	for(int jPt=0;jPt<BlockB_LO->NscalenodeScalePt;jPt++){
			...
			XSectionMuVar[i]	+=  BlockB_LO->SigmaTildeMuIndep[i][x][jQ][jPt][n] *                     as * pdflc;
			XSectionMuVar[i]	+=  BlockB_LO->SigmaTildeMuFDep [i][x][jQ][jPt][n] * std::log(muf2/Q2) * as * pdflc;
			XSectionMuVar[i]	+=  BlockB_LO->SigmaTildeMuRDep [i][x][jQ][jPt][n] * std::log(mur2/Q2) * as * pdflc;

			XSectionMuVar_LO[i]	+=  BlockB_LO->SigmaTildeMuIndep[i][x][jQ][jPt][n] *                     as * pdflc;
			XSectionMuVar_LO[i]	+=  BlockB_LO->SigmaTildeMuFDep [i][x][jQ][jPt][n] * std::log(muf2/Q2) * as * pdflc;
			XSectionMuVar_LO[i]	+=  BlockB_LO->SigmaTildeMuRDep [i][x][jQ][jPt][n] * std::log(mur2/Q2) * as * pdflc;
		      }
		      // NLO Block
		      for(int n=0;n<BlockB_NLO->NSubproc;n++){ 
			double as	= BlockB_NLO->AlphasTwoPi[i][jQ][jPt];
			double pdflc	= BlockB_NLO->PdfLcMuVar[i][x][jQ][jPt][n];
			XSectionMuVar[i]	+=  BlockB_NLO->SigmaTildeMuIndep[i][x][jQ][jPt][n] *                     as * pdflc;
			XSectionMuVar[i]	+=  BlockB_NLO->SigmaTildeMuFDep [i][x][jQ][jPt][n] * std::log(muf2/Q2) * as * pdflc;
			XSectionMuVar[i]	+=  BlockB_NLO->SigmaTildeMuRDep [i][x][jQ][jPt][n] * std::log(mur2/Q2) * as * pdflc;
