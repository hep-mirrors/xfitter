#!/bin/bash
### expect one input arguments: seed number

# write srteering cards, with changing number of systematic error 

function calc () {
#    gawk -v CONVFMT="%12.2f" -v OFMT="%.9g"  "BEGIN { print $* ; }"
    gawk -v CONVFMT="%12.2f" -v OFMT="%.8f"  "BEGIN { print $* ; }"
}
...
echo "got systematic error ${i}"

# now create cards file as tpolmc_${ipstr}_${epsstr}.cards
echo "producing steering file  steering.txt"
cat >  steering.txt <<EOF_CARDS
...
&InFiles
  ...
  ! Input files:
    InputFileNames(1) = 'ZEUS_LRG_98-00.dat'
    InputFileNames(2) = 'ZEUS_LPS_98-00.dat'    
&End
...
*
&CSOffset
  CorSysIndex  =  ${NrCorSys}
  
  ! -- Use previous fit results if exist in output dir
  !    0 - never use (default) 
  !    1 - use as input to the current fit (not implemented)
...
echo $i  
#\jobsub -h zenithsub  -qM eaze_run.sh -f steering.txt FitPDF ewparam.txt minuit.in.txt datafiles/hera/ZEUS_LRG_98-00.dat datafiles/hera/ZEUS_LPS_98-00.dat
